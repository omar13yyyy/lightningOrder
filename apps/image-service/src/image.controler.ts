...
getImage : async (req, res) => { //as stream
  try {
...
updateImage : async (req, res) => {
  try {
    ...
    const newFileName = await imageService.updateImage(req.query.fileName, req.file.buffer);
        //send file name to service 

    res.send({ message: 'Image updated successfully.', fileName: newFileName });
  } catch (err) {
    console.error(err);

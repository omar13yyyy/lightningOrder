...
export const customersController = {
  customerLogin: async (req, res) => {
    ...
    if (token != null) res.send({ token: token, massage: "Done" });
  ...
  customerRegister: async (req, res) => {
    ...
    if (token != null) {
      res.send({ token: token, massage: "Done" });
    }
    //TODO CODE 401
    else res.status(401).send({ token: null, massage: "not Register " });
  },
  ...
  confirmation: async (req, res) => {
    ...
    if (isNotExist) res.send({ success: true, massage: "Done" });
    //TODO CODE 400 ??
    else
      res
        .status(400)
        .send({ success: false, massage: "phone number is exist" });
  },
  resetConfirmation: async (req, res) => {
    ...
    if (isExist) res.send({ success: true, massage: "Done" });
    //TODO CODE 400 ??
    else
      res
        .status(400)
  ...
  checkCodeValidity: async (req, res) => {
    ...
    if (isValid)
      res.send({
        success: true,
        massage: `Valid Code for phone number : ${body.phoneNumber}`,
      });
    //TODO CODE 400 ??
    else
      res
        .status(400)
        .send({ success: false, massage: "Not Valid Code for phone number" });
  },
  resetPassword: async (req, res) => {
    ...
    if (isReset != null) {
      res.send({ success: true, massage: "Done" });
    }
    //TODO CODE 401
    else
      res.status(401).send({

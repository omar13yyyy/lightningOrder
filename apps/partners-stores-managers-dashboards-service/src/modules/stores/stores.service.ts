...
import {
  ...
  NearStoresByCategoryReq,
  NearStoresBytagRepo,
  NearStoresBytagService,
  NearStoresRepo,
  NearStoresService,
  OrderInputWithStoreId,
  SearchForStoreRepo,
...
function processRow(row): StoreItem {
  const delivery_price = roundUpToNearestThousand(
    DeliveryConfig.costPerKM * row.distance_km
  );
  const result: StoreItem = {
    ...
    status: row.status,
    delivery_price:
      delivery_price > DeliveryConfig.MinDeliveryCost
        ? delivery_price
        : DeliveryConfig.MinDeliveryCost,
...
//----------------------------------------------------------------------------------------------
  ...
  */
  ...
  getOrderItemsService: async (params: OrderInputWithStoreId) => {
    const { ar, en } = await storesRepository.getOrderItems({
      storeId: params.storeId,
    } as StoreIdRepo);
...
//--------------------------------------------------------
export function resolveOrderDetails(
  menu: MenuData,
  order: OrderInput
): TotalResolved {
  let totalPrice = 0;

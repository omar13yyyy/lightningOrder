...
const driversFilePath = path.join(rootLocation,"delivery"
, "btuidFiles", 'drivers.json');
export const driversGenerator = new BtuidGenerator({ path: driversFilePath });

const deliveryDocumentImagesFilePath = path.join(rootLocation,"delivery"
, "btuidFiles", 'delivery_document_images.json');
export const deliveryDocumentImagesGenerator = new BtuidGenerator({ path: deliveryDocumentImagesFilePath });

const driverTransactionsFilePath = path.join(rootLocation,"delivery"
, "btuidFiles", 'driver_transactions.json');
export const driverTransactionsGenerator = new BtuidGenerator({ path: driverTransactionsFilePath });

const driverWalletsPreviousDayFilePath = path.join(rootLocation,"delivery"
, "btuidFiles", 'driver_wallets_previous_day.json');
export const driverWalletsPreviousDayGenerator = new BtuidGenerator({ path: driverWalletsPreviousDayFilePath });

const trustPointsLogFilePath = path.join(rootLocation,"delivery"
, "btuidFiles", 'trust_points_log.json');

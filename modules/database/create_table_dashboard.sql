...
DROP TYPE IF EXISTS enum_store_transaction_type,
enum_day_of_week,
enum_store_status,
enum_orders_type,
enum_user_type,
...
CREATE TABLE
    coupons (
        ...
        description text,
        discount_value_percentage DOUBLE PRECISION CHECK (
            discount_value_percentage >= 0
            AND discount_value_percentage <= 1
        ),
...
CREATE TABLE
    store_wallets (
        store_id text, يا بتشيليو هاد و يلي تحتو ليصير لكل شريك و وقتا بدكن تتذكرو تحدثو كمان مع كل طلب 
        internal_store_id bigint,
        partner_id bigint,
        --previous day without platform_commission
...
CREATE TABLE
    IF NOT EXISTS public.permisions (
        id serial,
        title text,
        create_at timestamp
        with
...
CREATE TABLE
    daily_statistics (
        ...
        total_orders text,
        total_revenue text,
        average_delivery_time timestamp
        with
            time zone,
